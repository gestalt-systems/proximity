...
const SKIP: Skip = { skip: true, result: null }
...
export class DataCubeIndexer {
	private indexes: Map<MosaicClient, DataCubeInfo | Skip | null>
	private active: ActiveColumns | null
	 ...
	 * @param {Selection} selection A Mosaic selection
	 *  to filter the client by.
	 * @param {SelectionClause} activeClause
	...
	index(
		client: MosaicClient,
		selection: Selection,
		activeClause: SelectionClause
	): DataCubeInfo | Skip | null {
		...
		let info: DataCubeInfo | null | Skip
		if (!indexCols) {
...
interface ActiveColumns {
	source: string | null
	columns: { [key: string]: SQLExpression }
	predicate: (predicate: SQLExpression) => SQLExpression | SQLExpression[]
}
 ...
 * @param {SelectionClause} clause The
 *  active selection clause to analyze.
 */
function activeColumns(clause: SelectionClause): ActiveColumns {
	const { source, meta } = clause
	const clausePred = clause.predicate
	const clauseCols = clausePred?.columns
	let predicate: (predicate: SQLExpression) => SQLExpression | SQLExpression[]
	let columns: { [key: string]: SQLExpression }
	...
	if (type === "point") {
		...
		if (bins.some((b) => !b)) {
			...
			predicate = (p: SQLExpression) =>
				p ? isBetween("active0", (p as any).range.map(bins[0])) : []
			// @ts-ignore
			...
			predicate = (p: SQLExpression) =>
				p
					? and(
							(p as any).children.map(({ range }: any, i: number) =>
								isBetween(`active${i}`, range.map(bins[i]))
							)
						)
					: []
			columns = Object.fromEntries(
				// @ts-ignore
				clausePred.children.map((p: any, i: number) => [
					`active${i}`,
					bins[i](p.field),
				])
			)
 ...
 * @param {Scale} scale A scale that maps
 ...
 * @param {BinMethod} bin The binning
 *  method to apply, one of `floor`, `ceil', or `round`.
 * @returns {(value: any) => SQLExpression}
...
function binInterval(
	...
	pixelSize: number,
	bin: BinMethod
): ((value: any) => SQLExpression) | undefined {
	...
	return (value: any) => sql`${fn}(${s}(${sqlApply(value)}${d}))::INTEGER`
}

/**
 * Generate data cube table query information.
 * @param {Query} clientQuery The original client query.
 * @param {ActiveColumns} active Active (selected) column definitions.
 * @param {IndexColumns} indexCols Data cube index column definitions.
 * @returns {DataCubeInfo}
 */
function dataCubeInfo(
	clientQuery: Query,
	active: ActiveColumns,
	indexCols: IndexColumns
): DataCubeInfo {
	...
	const select = Query.select(dims, aggr)
		.from(table)
		.groupby(dims)
		.orderby(order)
 ...
 */
function subqueryPushdown(query: Query, cols: SQLExpression[]): void {
	const memo = new Set()
...
export class DataCubeInfo {
	 ...
	 * @param {SQLExpression} predicate The current
	 *  active clause predicate.
	 * @returns {Query} A data cube index table query.
	 */
	query(predicate: SQLExpression): Query {
